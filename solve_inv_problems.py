...
from guided_diffusion.semiblind_condition_methods import get_conditioning_method

from torch.utils.data import Subset  # Used to load a subset of the entire dataset
...
def main():
    ...
    # Data path for guidance samples used in Semiblind conditioning
    parser.add_argument('--guidance_path', type=str, default='./data/guidance_samples/random_images')  # TODO: Change 
    parser.add_argument('--n_guidance_samples', type=int, default=0)
    parser.add_argument('--n_gen_samples', type=int, default=1)
    ...
    # Working directory
    if args.n_guidance_samples > 0:
        out_path = os.path.join(args.save_dir, f"{measure_config['operator']['name']}_with_{args.n_guidance_samples}_guidance_samples")
    else:
    ...
    for i, ref_img in enumerate(loader):
        ...
        # Load guidance samples

        if args.n_guidance_samples > 0:
            guidance_samples_list = os.listdir(args.guidance_path)
            idx = np.random.randint(0, len(guidance_samples_list), size=(args.n_guidance_samples, ))
            guidance_samples = []
            for i in idx:
                guidance_samples.append(Image.open(os.path.join(args.guidance_path, guidance_samples_list[i])).convert('RGB'))

            # Convert the list into a tensor
            xs = torch.stack([transforms.ToTensor()(sample) for sample in guidance_samples]).to(device)
            print('xs shape = ', xs.shape)

            # Get ys from the measurement model with xs
            ys = operator.forward(xs, kernel)
            print('ys shape = ', ys.shape)
        else:
            xs = None
            ys = None

        measurement_cond_fn = partial(measurement_cond_fn, xs=xs, ys=ys)  # Add guidance samples
        sample_fn = partial(sampler.p_sample_loop, model=model, measurement_cond_fn=measurement_cond_fn)  # Set the sampler loop
